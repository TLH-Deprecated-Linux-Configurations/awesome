...
local title_color_dark = beautiful.xcolor0
local title_color_light = beautiful.xcolor15
...
_private.titlebar_color = beautiful.xcolor0
...
_private.context_menu_theme = {
  ...
  bg_normal = beautiful.xcolor0 .. "44",
  border_color = beautiful.xcolor0 .. "75",
  border_width = 20,
  -- fg_focus = "#fefefa",
  fg_normal = "#fefefa",
...
local function create_button_image(name, is_focused, event, is_on)
  ...
  if not _private[key_color] then
    ...
    button_color =
      (event == "hover") and colors.lighten(button_color, 45) or (event == "press") and colors.darken(button_color, 45) or
      button_color
...
local function create_titlebar_title(c)
  ...
  local function update()
    local text_color = is_contrast_acceptable(title_color_light, client_color) and title_color_light or title_color_dark
    title_widget.markup =
    ...
    function(c)
      ...
      c._cb_add_window_decorations = function()
        gtimer_weak_start_new(
          0.25,
          function()
            c._nice_base_color = untransparant(get_dominant_color(c) or _private.context_menu_theme.bg_normal)
            set_color_rule(c, c._nice_base_color)
            _private.add_window_decorations(c)
...
_G.tag.connect_signal(
  ...
  function(tag)
    ...
    for _, c in pairs(clients) do
      if (c.first_tag.layout.name ~= "max" and c.first_tag.layout.name ~= "floating") and not c.requests_no_titlebar then
        awful.titlebar.show(c, "top")
      else
        awful.titlebar.hide(c, "top")
      end
