...
return function(body, callback, bNo_center, leave_callback, no_update)
    ...
    button.bg = beautiful.bg_normal .. "cc"
    ...
    button.shape = function(cr, rect_width, rect_height)
        gears.shape.partially_rounded_rect(cr, rect_width, rect_height, true,
                                           true, true, true, 3)
    end
    ...
    -- ########################################################################
    -- ########################################################################
    if type(body) == "string" then body = wibox.widget.textbox(body) end
    -- ########################################################################
    -- ########################################################################
    -- ########################################################################
    ...
    -- ########################################################################
    -- ########################################################################
    button:connect_signal("mouse::enter", function() button.emulate_hover() end)
    -- ########################################################################
    -- ########################################################################
    -- ########################################################################
    button:connect_signal("mouse::leave",
                          function() button.emulate_focus_loss() end)
    -- ########################################################################
    -- ########################################################################
    -- ########################################################################
    if bNo_center then
        button:setup{layout = wibox.container.margin, body}
    else
        button:setup{layout = wibox.container.place, halign = "center", body}
    end
