...
local seperator_widget = function(height, orientation, opacity)
    height = height or 20
...
function mat_list_item:layout(_, width, height)
    ...
    if self._private.divider then
        table.insert(layout, base.place_widget_at(self._private.separator, 0, 0,
                                                  width, 1))
    end
    -- ########################################################################
    -- ########################################################################
    ...
    if self._private.clickable then
        table.insert(layout, base.place_widget_at(
                         self._private.clickable_container, 0, 0, width, height))
    end
    -- ########################################################################
    -- ########################################################################
    -- ########################################################################
    if self._private.prefix then
        ...
        table.insert(layout, base.place_widget_at(self._private.prefix, dpi(16),
                                                  0, dpi(48), height))
    end
    -- ########################################################################
    -- ########################################################################
    ...
    if self._private.suffix then
        content_width = content_width - dpi(54)
        table.insert(layout,
                     base.place_widget_at(self._private.suffix, width - dpi(40),
                                          dpi(12), width, height))
    end
    table.insert(layout, base.place_widget_at(self._private.content, content_x,
                                              0, content_width, height))
    return layout
end
-- ########################################################################
-- ########################################################################
-- ########################################################################
function mat_list_item:fit(_, width) return width, dpi(48) end
-- ########################################################################
-- ########################################################################
-- ########################################################################
---- Properties ----
...
-- ########################################################################
-- ########################################################################
function mat_list_item.mt:__call(...) return new(...) end
-- ########################################################################
-- ########################################################################
-- ########################################################################
