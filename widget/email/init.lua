...
local set_email_data_tooltip = function(email_data)
    ...
    local counter = "<span font='SF Intermosaic B  10'>Unread Count: </span>" .. unread_email_count
...
local set_latest_email_data = function(email_data)
    ...
    set_widget_markup(recent_from, recent_subject, recent_date)

    notify_new_email(unread_count, recent_from, recent_subject)
end

local set_empty_inbox_msg = function()
    set_widget_markup('empty@stdout.sh', 'Empty inbox', os.date('%d-%m-%Y %H:%M:%S'), 'Empty inbox.')
end
