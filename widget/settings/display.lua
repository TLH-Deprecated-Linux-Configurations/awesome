...
signals.connect_primary_theme_changed(function(pallete)
...
return function()
    ...
    local brightness = slider(0, 100, 1, 0, function(value)
        if _G.oled then
    ...
    local screen_time = slider(10, 600, 1, tonumber(general["screen_on_time"]) or 120, function(value)
    ...
    local changewall = button("Change wallpaper", function()
    ...
    local screenLayoutBtn = button("Screen Layout", function()
    ...
    monitors.update_body(wibox.widget {
    ...
    brightness_card.update_body(wibox.widget {
    ...
    screen_time_card.update_body(wibox.widget {
        layout = wibox.layout.fixed.vertical,
        {
            layout = wibox.container.margin,
            margins = m,
...
-- ########################################################################
    ...
    local function load_monitor(k, table, done, user)
        ...
        if filesystem.exists(v) then
            ...
            if filesystem.exists(scaledImage) and Display_Mode == WALLPAPER_MODE then
                ...
                imagemagic.scale(v, width, height, scaledImage, function()
                    if filesystem.exists(scaledImage) and Display_Mode == WALLPAPER_MODE then
                        layout:add(make_mon(scaledImage, k, v, true))
                    else
    ...
    local function render_normal_mode()
        ...
        awful.spawn.with_line_callback("tos theme active", {
            stdout = function(o)
            ...
            output_done = function()
                ...
                for k, v in pairs(screens) do
                    ...
                    if filesystem.exists(monitorScaledImage) then
                        ...
                        -- This is done to save memory consumption
                        -- However note that our cache (tempDisplayDir) is stored in ram
                        imagemagic.scale(v, width, height, monitorScaledImage, function()
                            layout:add(make_mon(monitorScaledImage, k, v))
                        end)
                    end
                end
    ...
    local function render_xrandr_mode()
        ...
        for _, tbl in ipairs(permutated_screens) do
            ...
            local screen_btn = button(widget, function()
                print("Executing: " .. cmd)
    ...
    refresh = function()
        ...
        awful.spawn.easy_async_with_shell("brightness -g", function(o)
