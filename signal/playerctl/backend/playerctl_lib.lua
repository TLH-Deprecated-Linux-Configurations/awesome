...
local function emit_metadata_signal(self, title, artist, artUrl, album, new, player_name)
    ...
    if artUrl ~= '' then
        local art_path = artUrl
        self:emit_signal('metadata', title, artist, art_path, album, new, player_name)
        capi.awesome.emit_signal('playerctl::title_artist_album', title, artist, art_path, player_name)
    else
...
local function metadata_cb(self, player, metadata)
    ...
    if player == self._private.manager.players[1] then
        ...
        if
            player ~= self._private.last_player or title ~= self._private.last_title or
                artist ~= self._private.last_artist or
                artUrl ~= self._private.last_artUrl
         then
...
local function playback_status_cb(self, player, status)
    ...
    if player == self._private.manager.players[1] then
        ...
        if status == 'PLAYING' then
            ...
            capi.awesome.emit_signal('playerctl::status', true, player.player_name)
        else
            self:emit_signal('playback_status', false, player.player_name)
            capi.awesome.emit_signal('playerctl::status', false, player.player_name)
        end
    end
end
...
local function start_manager(self)
    ...
    if #self.priority > 0 then
        self._private.manager:set_sort_func(
            function(a, b)
...
local function new(args)
    ...
    ret.update_on_activity = args.update_on_activity or beautiful.playerctl_update_on_activity or true
