                    ...
                    callback = function()
                        if title and title ~= '' then
                            if art_url ~= '' then
                                ...
                                capi.awesome.emit_signal('playerctl::title_artist_album', title, artist, '')
                            end
                        else
                ...
                function(length)
                    ...
                    if length_sec and interval_sec then
                        if interval_sec >= 0 and length_sec > 0 then
                            ...
                            capi.awesome.emit_signal('playerctl::position', interval_sec, length_sec / 1000000)
                        end
                    end
                end
            )
        ...
        status_cmd,
        {
            stdout = function(line)
                if line:find('Playing') then
                    ...
                    capi.awesome.emit_signal('playerctl::status', true)
                else
                    self:emit_signal('playback_status', false)
                    capi.awesome.emit_signal('playerctl::status', false)
                end
            end
        }
    )
