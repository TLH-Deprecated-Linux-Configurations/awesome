...
local secret_textbox =
    wibox.widget {
    font = beautiful.font_name .. 12,
    widget = wibox.widget.textbox
}

pass_textbox:connect_signal(
    'widget::redraw_needed',
    function()
        local secret = ' '
        if #pass_textbox.text > 1 then
            for i = 1, #pass_textbox.text - 1, 1 do
                secret = secret .. ''
            end
            secret_textbox.markup = secret .. ' '
        else
            secret_textbox.markup =
                "<span foreground='" .. beautiful.xforeground .. 55 .. "'><b><i>Password</i></b></span>"
        end
    end
)
...
awful.screen.connect_for_each_screen(
    function(s)
        if s == screen.primary then
            ...
            s.mylockscreen =
                helpers.screen_mask(s, gradient or beautiful.exit_screen_bg or beautiful.xbackground .. '80')
        end
    end
)
