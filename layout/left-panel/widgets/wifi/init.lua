...
local widget_button = clickable_container(wibox.container.margin(widget, dpi(3), dpi(3), 3, 3))
...
awful.tooltip(
    {
        ...
        margin_leftright = dpi(3),
        margin_topbottom = dpi(3)
    }
)
...
gears.timer {
    ...
    callback = function()
        ...
        local interface_name,
            _,
            link = interface_res:match('(%w+):%s+(%d+)%s+(%d+)')
