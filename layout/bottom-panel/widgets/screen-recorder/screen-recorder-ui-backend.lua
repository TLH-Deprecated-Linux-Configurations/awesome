...
local widget_icon_dir = config_dir .. "layouts/bottom-panel/screen-recorder/icons/"
local icons = require("theme.icons")

-- The screen-recorders scripting
local screen_rec_backend = require("layout.bottom-panel.widgets.screen-recorder.screen-recorder-scripts")

-- The screen-recorder's UI
local screen_rec_ui = require("layout.bottom-panel.widgets.screen-recorder.screen-recorder-ui")
...
sr_resolution_tbox:buttons(gears.table.join(awful.button({}, 1, nil, function()
	emphasize_inactive_tbox()
...
sr_offset_tbox:buttons(gears.table.join(awful.button({}, 1, nil, function()
	emphasize_inactive_tbox()
...
sr_settings_button:buttons(gears.table.join(awful.button({}, 1, nil, function()
	if not status_recording and not status_countdown then
		sr_navigation()
	end
end)))

sr_back_button:buttons(gears.table.join(awful.button({}, 1, nil, function()
	-- Save settings
	apply_new_settings()
...
sr_close_button:buttons(gears.table.join(awful.button({}, 1, nil, function()
	screen_rec_close()
end)))
...
local screen_close_on_rmb = function(widget)
	widget:buttons(gears.table.join(awful.button({}, 3, nil, function()
		screen_rec_close()
	end)))
end
...
sr_toggle_button:buttons(gears.table.join(awful.button({}, 1, nil, function()
	for s in screen do
		s.recorder_screen.visible = false
	end
...
sr_audio_button:buttons(gears.table.join(awful.button({}, 1, nil, function()
	sr_audio_mode()
end)))
...
sr_main_button:buttons(gears.table.join(awful.button({}, 1, nil, function()
	status_checker()
end)))
