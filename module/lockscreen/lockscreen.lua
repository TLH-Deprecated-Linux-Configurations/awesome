...
awful.screen.connect_for_each_screen(
    function(s)
        if s == screen.primary then
            ...
            s.mylockscreen =
                screen_mask(s, beautiful.lock_screen_bg or beautiful.exit_screen_bg or beautiful.xbackground)
        end
    end
)
...
local function activate_word(w)
    for i, m in pairs(char_map[w]) do
        local text = m.text
        m.markup = colorize_text(text, beautiful.xforeground)
    end
end
