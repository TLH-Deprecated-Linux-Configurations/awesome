...
local battery_function = require('module.battery')
...
gears.timer {
    ...
    callback = function()
        ...
        if value then signals.emit_battery(value) end
    end
}
-- ########################################################################
-- ########################################################################
...
signals.connect_battery(function(battery)
    local text
    local icon
    local timeout
    local urgency
    ...
    -- notification
    if text then
        send_notification(i18n.translate('Battery status'), text, icon, timeout,
                          urgency)
    end
end)
-- ########################################################################
...
signals.connect_battery_charging(function(plugged)
    ...
    if charger_first_time then
        ...
        send_notification(i18n.translate('Charger Status'), text, icon, 3,
                          urgency)
    end
end)
