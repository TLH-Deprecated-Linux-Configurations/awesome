...
-- ########################################################################
-- ########################################################################
-- These buttons are originally from https://github.com/streetturtle/awesome-buttons by streetturtle (a valuable asset to the awesomewm community)
-- modified to fit this configuration
...
buttons.with_icon = function(args)
    ...
    local color = args.color or '#f4f4f7'
    ...
    if icon:sub(1, 1) ~= '/' then
        icon = os.getenv('HOME') .. '/.config/awesome/theme/icons/' .. icon .. '.svg'
    end
...
buttons.with_icon_and_text = function(args)
    ...
    if type == 'outline' then
        result:set_shape_border_color('#8b9cbe')
        result:set_shape_border_width(2)
    end
