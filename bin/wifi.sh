...
val="$(printf "%s" "$networks" | rofi -dmenu -dpi "$DPI" -theme $HOME/.config/awesome/configuration/rofi/sidebar/rofi.rasi)" # get the requested network
...
password=$(printf "What is the password of %s?" "$val" | rofi -dmenu -dpi "$DPI" -password -theme $HOME/.config/awesome/configuration/rofi/sidebar/rofi.rasi) # get the requested dpi
