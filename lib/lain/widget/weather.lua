...
local function factory(args)
    ...
    local current_call =
        args.current_call or "curl -s 'https://api.openweathermap.org/data/2.5/weather?id=%s&units=%s&lang=%s&APPID=%s'"
    local forecast_call =
        args.forecast_call or
        "curl -s 'https://api.openweathermap.org/data/2.5/forecast?id=%s&units=%s&lang=%s&APPID=%s'"
    ...
    local notification_text_fun = args.notification_text_fun or function(wn)
            ...
            function(f)
                ...
                if not err and type(weather_now) == "table" and tonumber(weather_now["cod"]) == 200 then
                    ...
                    for i = 1, weather_now["cnt"], weather_now["cnt"] // cnt do
                        weather.notification_text =
                            weather.notification_text .. notification_text_fun(weather_now["list"][i])
