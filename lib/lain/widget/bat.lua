...
local function factory(args)
    ...
    if not fs.is_dir(pspath) then
        naughty.notify {text = "lib.lain.widget.bat: invalid power supply path", timeout = 0}
        return
    end
    ...
    function bat.update()
        ...
        for i, battery in ipairs(batteries) do
            ...
            if tonumber(present) == 1 then
                ...
                local energy_percentage =
                    tonumber(helpers.first_line(bstr .. "/capacity")) or math.floor((energy_now / energy_full) * 100)
