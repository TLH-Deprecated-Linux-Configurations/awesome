...
local function tasklist_update(s, w, buttons, filter, data, style, update_function, args)
    ...
    for _, c in ipairs(list) do
        if
            not (c.skip_taskbar or c.hidden or c.type == 'splash' or c.type == 'dock' or c.type == 'desktop') and
                filter(c, s)
         then
...
function tasklist.new(args, filter, buttons, style, update_function, base_widget)
    ...
    local spacing =
        args.style and args.style.spacing or args.layout and args.layout.spacing or beautiful.tasklist_spacing
