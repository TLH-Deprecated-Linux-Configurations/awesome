...
-- |   __ \.--.--.|  |--.---.-.|  |_.-----.
-- |      <|  |  ||  _  |  _  ||   _|  _  |
-- |___|__||_____||_____|___._||____|_____|
-- ########################################################################
-- ########################################################################
-- ########################################################################
-- Great worl available at: https://github.com/andOrlando/rubato
-- Placed in modules because I did have need to parse it and because organizational purposes
-- Keeping all animations in one place makes more sense in this case. 
-- AND I Might Do Some Modifications Of It In Tim
...
local bouncy = {
    F = (20 * math.pi - (10 * math.log(2) - 2049) * math.sqrt(3)) / (20 * math.pi - 20490 * math.sqrt(3) * math.log(2)),
    easing = function(t)
        -- short circuit
        if t == 0 then
            return 0
...
setmetatable(simulate_easing_mem, {
    __mode = "kv"
})
...
local function simulate_easing(pos, duration, intro, intro_e, outro, outro_e, m, b, dt)
    ...
    key = string.format("%f %f %f %s %f %s %s %s", pos, duration, intro, intro_e, outro, outro_e, m, b)
...
local function timed(obj)
    ...
    elseif not obj.outro then
        obj.outro = obj.intro
    end

    -- assert that these values are valid
    assert(obj.intro + obj.outro <= obj.duration or obj.prop_intro,
    ...
    timer:connect_signal("timeout", function()
        ...
        -- get dx
        dx = get_dx(time, obj.duration, (is_inter and obj.inter or obj.intro) * (obj.prop_intro and obj.duration or 1),
            is_inter and obj.easing_inter.easing or obj.easing.easing,
            obj.outro * (obj.prop_intro and obj.duration or 1), obj.easing_outro.easing, m, b)
    ...
    function obj:set(target_new)
        ...
        m = get_slope(is_inter and obj.easing_inter.F or obj.easing.F,
            ...
            obj.outro * (obj.prop_intro and obj.duration or 1), obj.duration, target - obj.pos,
            is_inter and obj.easing_inter.F or obj.easing.F, obj.easing_outro.F, b)

        if not override_simulate or b / math.abs(b) ~= m / math.abs(m) then
            ps_pos = simulate_easing(obj.pos, obj.duration,
                (is_inter and obj.inter or obj.intro) * (obj.prop_intro and obj.duration or 1),
                is_inter and obj.easing_inter.easing or obj.easing.easing,
                obj.outro * (obj.prop_intro and obj.duration or 1), obj.easing_outro.easing, m, b, dt)
    ...
    if obj.subscribed then
        obj:subscribe(obj.subscribed)
    end
