...
function wibox:to_widget()
    ...
    return wibox.widget {
        ...
        bg = self.bg or beautiful.bg_normal or '#f4f4f7',
...
function wibox:save_to_svg(path, context)
    wibox.widget.draw_to_svg_file(self:to_widget(), path, self:geometry().width, self:geometry().height, context)
end
...
function wibox:get_children_by_id(name)
    ...
    elseif self._drawable.widget and self._drawable.widget._private and self._drawable.widget._private.by_id then
...
object.properties._legacy_accessors(
    ...
    true,
    function(new_btns)
        return new_btns[1] and (type(new_btns[1]) == 'button' or new_btns[1]._is_capi_button) or false
    end,
    true
)
...
local function new(args)
    ...
    ret.drawin = w
    ret._drawable = wibox.drawable(w.drawable, {wibox = ret}, 'wibox drawable (' .. object.modulename(3) .. ')')
    ...
    mt.__tostring = function()
        return string.format('wibox: %s (%s)', tostring(ret._drawable), orig_string)
    end
