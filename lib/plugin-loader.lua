...
local function handle_plugin(value, name)
    ...
    if value:sub(1, 7) == 'widget.' then
        if general['minimize_network_usage'] == '1' and (value == 'widget.sars-cov-2' or value == 'widget.weather') then
    ...
    elseif dirExists(os.getenv('HOME') .. '/.config/awesome/plugins/' .. value) then
        ...
        if (plugin) then
            ...
            inValidPlugin(name, 'Errored out while loading. Make sure your plugins is the latest version.')
        end
    else
